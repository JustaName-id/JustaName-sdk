---
id: ens-architecture
title: Deep dive into ENS architecture
sidebar_label: Deep dive into the ENS architecture
...
- **Registrar:** A Registrar is a smart contract that owns a top-level domain (like .eth) and creates second-level domains (like 'yourname.eth'). It has the ability to define the rules and mechanisms for the assignment of these subdomains.
...
- **Users:** Users are individuals or entities that interact with the ENS, whether by registering a new domain name, updating the resources linked to a domain name, or querying a domain name to fetch its linked resources.
