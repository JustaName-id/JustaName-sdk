...
export type FullstackFramework = 'nextjs' | 'expo';
...
export const collectAppDetails = async (): Promise<AppDetails> => {
  ...
  const frontendFramework = await select<
    FrontendFramework | FullstackFramework
  >({
    choices: [
      {
        name: 'React',
  ...
  if (frontendFramework === 'expo') {
    expoWithApiRoutes = await confirm({
      message: 'Do you want to use API routes?',
      default: false,
    });
  }

  if (frontendFramework !== 'nextjs') {
    if (!expoWithApiRoutes) {
  ...
  const appUrl = await input({
    message:
      'Enter the URL of your app (example: https://<appName>.com or http://localhost:3000)',
  ...
  if (frontendFramework !== 'nextjs' && !expoWithApiRoutes && !backendFramework)
    throw new Error('Backend framework is required');

  return {
    appName,
    frontendFramework:
      frontendFramework === 'nextjs' || expoWithApiRoutes
        ? undefined
        : frontendFramework,
    backendFramework:
      frontendFramework === 'nextjs' || expoWithApiRoutes
