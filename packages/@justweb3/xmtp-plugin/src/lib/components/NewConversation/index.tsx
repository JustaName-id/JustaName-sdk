...
const NewConversation: React.FC<NewConversationProps> = ({
  ...
  const { debouncedValue: debouncedAddress } = useDebounce<string>(
    newAddress,
    500
  );
  ...
  const { records, isRecordsLoading } = useRecords({
    ens: !isAddressName ? debouncedAddress : undefined,
  });

  const { primaryName: name, isPrimaryNameLoading } = usePrimaryName({
    address: isAddressName ? debouncedAddress : undefined,
  });

  // const { name, isRecordsLoading } = useAddressResolutionName(debouncedAddress, !isAddressName);
  //
  //   const { address: resolvedAddress, isPrimaryNameLoading } = useIdentityResolution(debouncedAddress, isAddressName);
  ...
  useEffect(() => {
    if (!isLoading && !isPrimaryNameLoading && debouncedAddress.length > 0) {
      handleCanMessage();
    } else if (canMessage && debouncedAddress.length === 0) {
      setCanMessage(false);
    }
  }, [debouncedAddress, isLoading, isPrimaryNameLoading]);
    ...
    >
      <Flex direction="row" align="center" gap="10px" justify="space-between">
        ...
        <Input
          ...
          className="flex-1"
          left={
            <p className="font-poppins text-primary text-base font-bold leading-[150%]">
              To
            </p>
          }
          right={
            isSearchLoading ? (
              ...
              <CancelIcon
                ...
                height={24}
                className={`${
                  canMessage ? 'text-green-600' : 'text-red-600'
                } cursor-pointer`}
                onClick={() => {
                  setNewAddress('');
                }}
