...
import { coinTypeMap, getCoinTypeDetails, SupportedCoins } from '@justaname.id/sdk';
...
import { AddIcon, Button, Flex, P, WalletIcon, Input, DropdownMenu, DropdownMenuTrigger, DropdownMenuContent } from '@justweb3/ui';
...
export const AddressesSection: React.FC<AddressesSectionProps> = ({ form }) => {
  ...
  const [chainDropdownOpen, setChainDropdownOpen] = React.useState<boolean>(false);
  const {
    debouncedValue: debouncedAddress,
    isDebouncing
  } = useDebounce(address, 500);
  ...
  const filteredSuggestedCoins = useMemo(() => {
    return suggestedCoins.filter((coin) => !form.getValues('addresses').some((address) => address.coin === coin));
    ...
    <Container>
      <Flex direction="row" gap="10px" align="center">
        ...
        <Flex direction="column" gap="0px">
          ...
          <P style={{ fontSize: '16px', fontWeight: 700, color: 'black' }}>Addresses</P>
        </Flex>
      </Flex>
        ...
        <Flex direction='column' gap='10px'>
          {
            form.getValues('addresses').map((address, index) => {
              ...
              const coinDetails = getCoinTypeDetails(address.coin as SupportedCoins);
              return (
                <MetadataField
                  key={index}
                  ...
                  onDelete={address.coin === '60' ? undefined : () => {
                    remove(index);
                  }}
                />
              );
          ...
          <Input
            ...
            right={
              <DropdownMenu open={chainDropdownOpen} onOpenChange={(open) => setChainDropdownOpen(open)}>
                <DropdownMenuTrigger disabled={filteredSuggestedCoins.length === 0}>
                  <Flex direction='row' gap='4px' align='center' style={{
                    ...
                    {selectedCoin ?
                      <Flex direction='row' gap='10px' align='center'>
                        ...
                        <P style={{
                          ...
                          {`${selectedCoinDetails.symbol.split('Legacy')[0]} ${selectedCoinDetails.symbol.includes('Legacy') ? "Legacy" : ""}`}
                        </P>
                      </Flex>
                      :
                      <>
                ...
                <DropdownMenuContent style={{
                ...
                }}>
                  {
                    filteredSuggestedCoins?.map((coin) => {
                      const coinDetails = getCoinTypeDetails(coin as SupportedCoins);
                      return (
                        <ChainCard isNotAllowed={form.getValues('addresses').some((address) => address.coin === coin)} onClick={() => {
                          ...
                          <P style={{
                            ...
                            {`${coinDetails.symbol.split('Legacy')[0]} ${coinDetails.symbol.includes('Legacy') ? "Legacy" : ""}`}
                          </P>
                        </ChainCard>
                      );
                    })
