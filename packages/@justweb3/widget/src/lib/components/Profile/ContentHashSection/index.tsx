...
import { ContentHashProvider, validateContentHash } from '../../../forms/contenthash.schema';
import { MetadataField } from '../MetadataField';
import { useDebounce } from '../../../hooks';
import { metadataForm } from '../../../forms';
import { ContentHashProviders } from '../../../forms/contenthash.schema';
...
export const ContentHashSection: React.FC<ContentHashSectionProps> = ({ form }) => {

  const contentHashesRef = React.useRef<HTMLDivElement>(null);
  const [contentHash, setContentHash] = React.useState<string>('');
  const [selectedContentHashProtocol, setSelectedContentHashProtocol] = React.useState<string>('');
  const {
    debouncedValue: debouncedContentHash,
    isDebouncing
  } = useDebounce(contentHash, 500);
  ...
  const suggestedContentHashProtocols = useMemo(() => {
    ...
    return ContentHashProviders.reduce((acc: string[], provider: ContentHashProvider) => {
      if (
        validateContentHash(provider)(debouncedContentHash) === true
      ) {
        acc.push(provider);
      }
      return acc;
    ...
    <Container>
      <Flex direction="row" gap="10px" align="center">
        ...
        <Flex direction="column" gap="0px">
          ...
          <P style={{ fontSize: '16px', fontWeight: 700, color: 'black' }}>ContentHash</P>
        </Flex>
      </Flex>

      {
        form.getValues('contentHash').length > 0 ?
          <MetadataField
            ...
            form={form}
            leftIcon={getContentHashIcon(form.getValues('contentHash')[0].protocolType)}
          ...
          <Flex direction='column' gap='10px' style={{
            ...
            <Flex direction='column' gap='20px'>
              ...
              <Flex direction='row' gap='10px' align='center' style={{ flexWrap: 'wrap' }}>
                {
                  suggestedContentHashProtocols?.map((contentHashProcotol) => {

                    return (
                      <ContentHashCard
                        isSelected={selectedContentHashProtocol === contentHashProcotol}
                        onClick={() => {
                      ...
                      >
                        <Flex direction='column' gap='10px' align='center' style={{ maxWidth: '100px' }}>
              ...
              {
                !isDebouncing && debouncedContentHash && suggestedContentHashProtocols.length === 0 &&
                <P style={{
                  color: 'var(--justweb3-primary-color)',
                  fontSize: '12px',
                  fontWeight: 500,
              ...
              <Button
                ...
                onClick={() => {
                  append({
                    ...
                    decoded: debouncedContentHash.split('://').length > 1 ?
