...
export interface UseRejectSubnameFunctionParams
  extends Omit<SubnameRejectRoute['params'], 'ensDomain' | 'username'> {
  ens: string;
}

export type UseRejectSubnameParams = Omit<
  UseRejectSubnameFunctionParams,
  'ens'
>;

export interface UseRejectSubnameResult {
  rejectSubname: UseMutateAsyncFunction<
    Records,
    Error,
    UseRejectSubnameFunctionParams
  >;
...
export const useRejectSubname = (
  params?: UseRejectSubnameParams
