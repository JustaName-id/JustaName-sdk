...
export interface JustSignButtonProps {
  children: ReactNode;
}

export const JustSignInButton: FC<JustSignButtonProps> = ({
                                                              children
                                                            }) => {
  const [openMApps, setOpenMApps] = useState(false);
  const { plugins, mApps } = useContext(JustSignInContext)
  const { createPluginApi } = useContext(PluginContext)
  const { address } = useMountedAccount()
  const { connectedEns, signOut, isEnsAuthPending, handleOpenSignInDialog } = useSignInWithJustaName();
